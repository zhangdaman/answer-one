...
> **一稿多版（内容资产包）**：同一缺口可切换多个平台分别生成——标题 20↔100 字、视频 9:16↔16:9 不能靠截断 / 裁剪，必须按平台各出一版，组成「内容资产包」。平台清单（5 个）见 §9.3 `publish_platform`，与诊断的 AI 平台·端是两套概念。
...
> 汇总 AI 回答中实际引用的站外来源，回答"想被 AI 引用，该去哪铺内容"。每项：`title`（来源标题）/ `url`（原文链接）/ `source_type`（来源类型，见 §9.3）/ `platforms`（被哪些 AI 平台·端引用）/ `cite_share`（被引用占比 0-1）。因站外第三方来源被引用概率远高于自有域，信源分析直接喂给"生成 / 分发"——告诉学员优先去哪些占比高、自己还没覆盖的渠道铺内容。
...
| publish_platform 发布平台（内容生成目标，≠ 诊断的 AI 平台·端） | xiaohongshu 小红书 / douyin 抖音 / dianping 大众点评 / gzh 公众号 / zhihu 知乎 |
