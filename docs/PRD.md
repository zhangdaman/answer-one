...
> 学员登录、填写品牌信息、点"开始诊断"，几分钟后看到一份报告："你在豆包里被提及 0 次、千问里 2 次……"——学员第一次看清自己在 AI 里的真实状态。
...
2. 将这组问题**逐一发送给所选的多个 AI 平台**
3. 收集每个 AI 的回答
4. 在每个回答里，**检测品牌名是否被提及、提及位置、上下文情感**
5. 汇总成报告

** V1 接入的 AI 平台**（8 个，不区分手机 / 网页端）：
- 千问、豆包、DeepSeek、腾讯元宝、文心、KIMI、百度AI、AI抖音
- 走平台官方 API（API 不区分端）；无公开 API 的平台（腾讯元宝 / 百度AI / AI抖音）用备用算法估算。选择标准：用户基数大、行业相关

#### 3.1.3 输出

**一份《GEO 诊断报告》**，包含：
- **总分**：综合 0-100 分（提及率 × 排名 × 情感 加权）
- **四项关键指标**：提及率、**首位提及率**（被提及时排在首位的比例，比"是否被提及"更能反映"优先推荐力"）、平均提及位置、正面情感率
- **上榜要素诊断**：「被检索 → 被引用 → 优先推荐」可见度漏斗，外加 5 个公开研究验证的上榜要素评分（相关性 / 具体性 / 权威引用 / 内容长度 / 第三方提及），解释"为什么被/不被引用"并导向生成建议（打分模型见 §9.2）
- **信源分析**：AI 回答主要引用了哪些站外来源（来源标题 / 类型 / 平台分布 / 被引用占比 / 原文链接）。这是 GEO 的"供给侧"洞察——知道 AI 从哪取数，就知道该去哪铺内容，直接衔接"生成 / 分发"（字段见 §9.2 `sources`）
- **竞品对标**：当前品牌 vs 同赛道竞品的提及率 / 首位提及率 / 正面率排行榜，叠加"竞争位置分布"，看清相对位置（字段见 §9.2 `competitors`）
- **平台明细**：每个 AI 平台单独的提及次数、问题清单、回答原文摘录
- **缺口诊断**：哪些问题/平台里完全 0 提及（标红，作为下一步"生成"的输入）
- 报告支持**在线查看 + 下载 PDF**

#### 3.1.4 关键体验

- 学员点"开始诊断"后看到**进度条**（"正在询问豆包… 3/10"），避免黑盒等待
...
**目标平台（V1 新增 · 决定格式布局）**：选定内容要发到哪个**发布平台**——这些平台正是诊断「信源分析」里 AI 取数的来源。平台一旦选定，自动套用该平台的**格式预设**，不同平台版式不同（不再「一份内容走天下」）：

| 平台 | 主推形态 | 图片比例 | 标题字数 | 正文字数 | 标签语法 | 灵魂格式 |
|---|---|---|---|---|---|---|
| 小红书 | 图文 | 3:4 ×≤9 | ≤20 | ≤1000 | `#话题#` ×10 | emoji 标题 + 9 图集，封面定生死 |
| 抖音 | 视频 | 9:16 | ≤55 | ≤2200 | `#话题` ×10 | 竖屏 15–60s，前 3 秒钩子 + 全屏字幕 |
| 大众点评 ⚠️ | 图文 | 4:3 | ≤30 | ≤1000 | 关键词 ×5 | 评分 + 人均 + 推荐菜 + 挂 POI |
| 公众号 | 长图文 | 2.35:1 封面 | ≤64 | 无硬上限（甜区 1.5–3k） | 无原生标签 | 头条封面 + 小标题分段 + 文末关注 |
| 知乎 | 长答案 | 16:9 | ≤100 | 无上限（甜区 1–5k） | 关键词 ×5 | 论据 + 数据 + 个人经历四件套 |

> ⚠️ 大众点评规格为经验值，落地前需人工核验。比例 / 字数 / 标签语法属 **L1 硬约束**（生成时必满足；错平台标签＝明文乱码掉权重），灵魂格式属 **L2 质量提示**——只把 L1+L2 塞进生成 prompt，避免 token 爆炸。
> **一稿多版（内容资产包）**：同一缺口可切换多个平台分别生成——标题 20↔100 字、视频 9:16↔16:9 不能靠截断 / 裁剪，必须按平台各出一版，组成「内容资产包」。平台清单（5 个）见 §9.3 `publish_platform`，与诊断的 AI 平台是两套概念。
...
| target_channels | enum[] | 目标 AI 平台（platform 值，见 9.3；端已合并） |
...
| channels | enum[] | 本次目标平台（platform 值） |
| progress | string | 进度，如 "3/10" |
| score_total | int (0-100) | 总分 |
| metric_mention_rate | float | 提及率 |
| metric_first_mention_rate | float | 首位提及率（被提及中排在首位的比例） |
...
> **信源分析 sources（报告 payload，落 api-contracts `sources[]`）**
> 汇总 AI 回答中实际引用的站外来源，回答"想被 AI 引用，该去哪铺内容"。每项：`title`（来源标题）/ `url`（原文链接）/ `source_type`（来源类型，见 §9.3）/ `platforms`（被哪些 AI 平台引用）/ `cite_share`（被引用占比 0-1）。因站外第三方来源被引用概率远高于自有域，信源分析直接喂给"生成 / 分发"——告诉学员优先去哪些占比高、自己还没覆盖的渠道铺内容。

> **竞品对标 competitors（报告 payload，落 api-contracts `competitors[]`）**
> 同一批问题里被共同提及的同赛道品牌排行。每项：`name` / `is_current`（是否当前品牌，用于高亮）/ `mention_rate` / `first_mention_rate` / `positive_rate`。V1 自动从回答中识别同赛道竞品（与当前品牌在同类问题下被并列提及者）；手动指定竞品列为后续版本。

#### Mention（提及结果，问题 × 平台明细）

| 字段 | 类型 | 说明 |
|---|---|---|
| id / diagnosis_id / question_id | string | 主键 / 所属诊断 / 所属问题 |
| platform | enum | AI 平台（见 9.3 `platform`） |
...
| target_platform | enum | 目标发布平台（见 9.3 `publish_platform`）；决定图片比例 / 标题·正文字数 / 标签语法等格式预设，同一缺口可生成多平台版本 |
| body | text | 正文（仅文字） |
...
| publish_platform 发布平台（内容生成目标，≠ 诊断的 AI 平台） | xiaohongshu 小红书 / douyin 抖音 / dianping 大众点评 / gzh 公众号 / zhihu 知乎 |
...
| source_type 信源类型 | 点评平台 / 社交媒体 / 问答社区 / 百科 / 自媒体 / 新闻媒体 / 自有渠道 |

**AI 平台 platform（V1 共 8 个，诊断的统计单位）**：`qwen` 千问 / `doubao` 豆包 / `deepseek` DeepSeek / `yuanbao` 腾讯元宝 / `wenxin` 文心 / `kimi` KIMI / `baidu` 百度AI / `douyin` AI抖音

> **不区分手机版 / 网页版**：诊断走平台官方 API，API 不区分端，故按「平台」统计（早期曾把每个平台再分手机 / 网页两端、共 12 个，现合并为 8 个平台）。每个平台标记接入方式：有官方 API → `api`（千问 / 豆包 / DeepSeek / 文心 / KIMI）；无 API → `fallback`（腾讯元宝 / 百度AI / AI抖音，备用采集算法估算，方案开发前定）。平台与接入方式以数据库字典表维护，便于增删。
